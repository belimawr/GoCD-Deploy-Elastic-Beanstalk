...
def wait_deployment(client=None, **kwargs):
    ...
    status = ''
    colour = ''
    older_version = client.describe_environments(
        EnvironmentNames=[kwargs['EnvironmentName'], ])
    older_version = older_version['Environments'][0]['VersionLabel']
    while not (colour == 'Green' and status == 'Ready'):
        ...
        time.sleep(5)
    current_version = client.describe_environments(
        EnvironmentNames=[kwargs['EnvironmentName'], ])
    current_version = current_version['Environments'][0]['VersionLabel']
    if current_version == older_version:
        print 'Deployment aborted, running version is: %s' % current_version
        sys.exit(1)
